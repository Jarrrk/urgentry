...
func RegisterAnalyticsRoutes(mux *http.ServeMux, routes AnalyticsRoutes) {
	...
	if routes.BlobStore != nil {
		mux.Handle("GET /api/0/projects/{org_slug}/{proj_slug}/replays/{replay_id}/recording-events/", handleGetReplayRecordingEvents(routes.DB, routes.Queries, routes.BlobStore, routes.WithAuth(auth.Policy{Scope: auth.ScopeProjectRead, Resource: auth.ResourceProjectPath})))
